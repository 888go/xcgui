...
// 检索指定模块句柄。 模块必须已由调用进程加载。
// //详情: https://learn.microsoft.com/zh-cn/windows/win32/api/libloaderapi/nf-libloaderapi-getmodulehandlew.
// //@param lpModuleName: 已加载模块名称 (.dll 或 .exe 文件) 。 如果省略文件扩展名，则会追加默认库扩展名 .dll。 文件名字符串可以包含尾随点字符 (.) ，以指示模块名称没有扩展名。 字符串不必指定路径。 指定路径时，请务必使用反斜杠 (\) ，而不是使用 /) (正斜杠。 名称 (大小写独立比较，) 当前映射到调用进程的地址空间的模块的名称。如果此参数为空， 则 GetModuleHandleW 返回用于创建调用进程 (.exe 文件) 的文件的句柄。
func GetModuleHandleW(lpModuleName string) uintptr {
	r, _, _ := getModuleHandleW.Call(common.StrPtr(lpModuleName))
	return r
}

// 延时, 暂停当前线程的执行，直到超时间隔结束。若要进入可警报等待状态，请使用 SleepEx 函数。
...
// 延时EX，直到满足指定的条件。 发生以下情况之一时，将继续执行：
// //- 调用 I/O 完成回调函数。
// - 异步过程调用 (APC) 排队到线程。
// - 超时间隔已过。
// //详情: https://learn.microsoft.com/zh-cn/windows/win32/api/synchapi/nf-synchapi-sleepex.
// dwMilliseconds: 延时毫秒（以毫秒为单位）。
//
//	如果发生 I/O 完成回调，该函数不会立即返回，并且不会执行 I/O 完成函数。 如果 APC 已排队到线程，该函数不会立即返回，并且不会执行 APC 函数。如果 参数为 TRUE，并且调用此函数的线程与调用扩展 I/O 函数 (ReadFileEx 或 WriteFileEx) 的线程相同，则当超时期限已过或发生 I/O 完成回调函数时，函数将返回 。 如果发生 I/O 完成回调，则调用 I/O 完成函数。 如果将 APC 排队到 queueUserAPC) (线程，则当超时期限已过或调用 APC 函数时，函数将返回 。
//
...
// @param dwBytes: 字节数。如果此参数为0并且uFlags参数指定 GMEM_Moveable ，则该函数返回标记为已丢弃的内存对象的句柄.
...
// 全局内存对象解锁 减少与使用 GMEM_Moveable 分配的内存对象关联的锁计数。此函数对使用 GMEM_Fixed 分配的内存对象没有影响.
...
// 全局内存对象取大小 检索指定全局内存对象的当前大小，以字节为单位.
...
//
// @param lpString1:文本指针1 用于接收 lpString2 参数指向的字符串内容的缓冲区. 缓冲区必须足够大以包含字符串，包括终止空字符.
// @param lpString2:文本指针2 要复制的以 null 结尾的字符串.
...
// 全局内存对象关闭 释放指定的全局内存对象并使其句柄无效.
// //@Description 详情: https://docs.microsoft.com/zh-cn/windows/win32/api/winbase/nf-winbase-GlobalFree.
//
// @param hMem: 内存对象句柄. 此句柄由 GlobalAlloc 或 GlobalReAlloc 函数返回. 释放使用 LocalAlloc 分配的内存是不安全的.
