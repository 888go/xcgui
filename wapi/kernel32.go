...
func X检索指定模块句柄(已加载模块名称 string) uintptr {
	r, _, _ := getModuleHandleW.Call(炫彩工具类.StrPtr(已加载模块名称))
	return r
}
...
func X延时EX(毫秒 uint32, bAlertable bool) uint32 {
	r, _, _ := sleepEx.Call(uintptr(毫秒), 炫彩工具类.BoolPtr(bAlertable))
	return uint32(r)
}
...
func X全局内存分配(内存分配属性 GMEM_, 字节数 uint) uintptr {
	r, _, _ := globalAlloc.Call(uintptr(内存分配属性), uintptr(字节数))
	return r
}
...
func X全局内存对象解锁(内存对象句柄 uintptr) bool {
	r, _, _ := globalUnlock.Call(内存对象句柄)
	return r != 0
}
...
func X复制到缓冲区(文本指针1, 文本指针2 uintptr) uintptr {
	r, _, _ := lstrcpyW.Call(文本指针1, 文本指针2)
	return r
}
