...
// hEle:元素句柄    .元素句柄      元素句柄.
// hXCGUI:对象句柄      .对象句柄, UI元素句柄或形状对象句柄.
...
// 列表_置锁定列右侧.
// hEle:元素句柄    .元素句柄      元素句柄.
// iColumn:列索引         .     列索引      列索引, -1代表不锁定. 暂时只支持锁定末尾列.
...
// 列表_测试点击项EX, 检查坐标点所在项, 自动添加滚动视图偏移量.
// hEle:元素句柄    .元素句柄      元素句柄.
// pPt:坐标点.
// piItem:项索引.
// piSubItem:子项索引.
...
// hEle:元素句柄    .元素句柄      元素句柄.
// nWidth:列宽.
// pName:名称     .名称       模板绑定名称. 在List内部存在有默认模板, name名是从name1到namen. 你可以理解为创建表头数据适配器后, 内部有一个Map来存储每一列的表头名(列名), 这个name名就是Map的Key, 这个函数就相当于给每一列的Key赋值, 然后List会根据这个name名从Map读取Value来显示表头到界面.
// pText:文本     .  文本.
