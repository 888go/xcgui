...
func X消息框(标题, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, hWndParent uintptr, XCStyle 炫彩常量类.Window_Style_) 炫彩常量类.MessageBox_Flag_ {
	r, _, _ := xC_MessageBox.Call(炫彩工具类.StrPtr(标题), 炫彩工具类.StrPtr(内容文本), uintptr(标识), hWndParent, uintptr(XCStyle))
...
func X消息框_创建(标题, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, hWndParent uintptr, XCStyle 炫彩常量类.Window_Style_) int {
	r, _, _ := xMsg_Create.Call(炫彩工具类.StrPtr(标题), 炫彩工具类.StrPtr(内容文本), uintptr(标识), hWndParent, uintptr(XCStyle))
	return int(r)
}
...
func X消息框_创建EX(窗口扩展样式 int, 窗口样式 int, 窗口类名, 标题, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, hWndParent uintptr, XCStyle 炫彩常量类.Window_Style_) int {
	r, _, _ := xMsg_CreateEx.Call(uintptr(窗口扩展样式), uintptr(窗口样式), 炫彩工具类.StrPtr(窗口类名), 炫彩工具类.StrPtr(标题), 炫彩工具类.StrPtr(内容文本), uintptr(标识), hWndParent, uintptr(XCStyle))
	return int(r)
}
...
func X取文本显示大小EX(字符串 string, 字符串长度 int, 字体 int, 文本对齐方式 炫彩常量类.TextFormatFlag_, pOutSize *SIZE) int {
	r, _, _ := xC_GetTextShowSizeEx.Call(炫彩工具类.StrPtr(字符串), uintptr(字符串长度), uintptr(字体), uintptr(文本对齐方式), uintptr(unsafe.Pointer(pOutSize)))
	return int(r)
}
...
func X炫彩_取文本显示矩形(字符串 string, 字符串长度 int, 字体 int, 文本对齐 炫彩常量类.TextFormatFlag_, width int, pOutSize *SIZE) int {
	r, _, _ := xC_GetTextShowRect.Call(炫彩工具类.StrPtr(字符串), uintptr(字符串长度), uintptr(字体), uintptr(文本对齐), uintptr(width), uintptr(unsafe.Pointer(pOutSize)))
	return int(r)
}
