...
func X编辑框_创建EX(x坐标 int, y坐标 int, 宽度 int, 高度 int, 类型 炫彩常量类.Edit_Type_, hParent int) int {
	r, _, _ := xEdit_CreateEx.Call(uintptr(x坐标), uintptr(y坐标), uintptr(宽度), uintptr(高度), uintptr(类型), uintptr(hParent))
	return int(r)
}
...
func X编辑框_添加样式EX(元素句柄 int, 字体名称 string, 字体大小 int, 字体样式 炫彩常量类.FontStyle_, color int, bColor bool) int {
	r, _, _ := xEdit_AddStyleEx.Call(uintptr(元素句柄), 炫彩工具类.StrPtr(字体名称), uintptr(字体大小), uintptr(字体样式), uintptr(color), 炫彩工具类.BoolPtr(bColor))
	return int(r)
}
