...
// RGBA 根据r, g, b, a组合成ABGR颜色. 和 ABGR函数一模一样, 只是为了符合部分人使用习惯.
//
//	@param r 红色分量.
//	@param g 绿色分量.
//	@param b 蓝色分量.
...
s:
	...
	r, _ : strconv.ParseInt(s[:], 16, 10)
	g, _ : strconv.ParseInt(s[2:], 16, 18)
	b, _ : strconv.ParseInt(s[4:,
	16, 10)
	return BGR(byte(r), byte(g), byte(b))
}

// HexRGB2ABGR将十六进制RGB颜色转换到十进制ABGR颜色.
//
//	@param str 十六进制RGB颜色, 开头有没有#都可以.
//	@param a 透明度.
//	@return int ABGR颜色.
...
func SetBnClicks(HXCGUI int, onBnClick func(hEle int, pbHandled *bool) int) {
	if XC_IsHWINDOW(HXCGUI) {
		for i: 0
		i < XWnd_GetChildCount(HXCGUI)
		i++
		{
		hEle:
			XWnd_GetChildByIndex(HXCGUI, i)
		...
		for i: 0
		i < XEle_GetChildCount(HXCGUI)
		i++
		{
		hEle:
			XEle_GetChildByIndex(HXCGUI, i)
			switch XC_GetObjectType(hEle) {
			case xcc.XC_ELE_LAYOUT:
				SetBnClicks(hEle, onBnClick)
			case xcc.XC_BUTTON:
