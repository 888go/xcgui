...
// hRedrawObjectUI:重绘的UI层. UI对象句柄:窗口句柄, 元素句柄, 形状句柄, SVG句柄.
...
// 动画_释放EX, 释放与指定UI对象关联的所有动画, 返回释放动画数量.
// hObjectUI:指定UI对象句柄.
// bEnd:是否立即执行到终点.
...
// 动画_移动EX, 从指定位置移动到目标位置, 默认以UI对象中心点为操作方式, 避免出现坐标错位, 返回动画项句柄.
// hSequence:动画序列句柄.
// duration:持续时间.
...
// 动画_延迟EX, 可以作为一个空动画, 然后在回调里处理自己的算法, 返回动画项句柄.
// hSequence:动画序列句柄.
// duration:持续时间.
// nLoopCount:动画循环次数, 0:限循环.
// ease_flag:缓动标识, Ease_Flag_.
