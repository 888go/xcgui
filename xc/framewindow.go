...
func XFrameWnd_Create(x int, y int, cx int, cy int, pTitle string, hWndParent int, XCStyle xcc.I常量_窗口样式_) int {
...
//
// XCStyle: GUI库窗口样式: I常量_窗口样式_.
func XFrameWnd_CreateEx(dwExStyle int, dwStyle int, lpClassName string, x int, y int, cx int, cy int, pTitle string, hWndParent int, XCStyle xcc.I常量_窗口样式_) int {
...
func XFrameWnd_AddPane(hWindow int, hPaneDest int, hPaneNew int, align xcc.I常量_框架窗口对齐_) bool {
...
//
// hWindow: 窗口句柄.
func XFrameWnd_GetDragFloatWndTopFlag(hWindow int) xcc.I常量_框架窗口单元格类型_ {
	r, _, _ := xFrameWnd_GetDragFloatWndTopFlag.Call(uintptr(hWindow))
	return xcc.I常量_框架窗口单元格类型_(r)
}
