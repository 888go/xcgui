...
func X菜单_插入项(菜单句柄 int, 项ID int32, 文本内容 string, 标识 炫彩常量类.Menu_Item_Flag_, insertID int32) {
	xMenu_InsertItem.Call(uintptr(菜单句柄), uintptr(项ID), 炫彩工具类.StrPtr(文本内容), uintptr(标识), uintptr(insertID))
}
...
func X菜单_插入项图标(菜单句柄 int, 项ID int32, 文本内容 string, 菜单项图标句柄 int, 标识 炫彩常量类.Menu_Item_Flag_, insertID int32) {
	xMenu_InsertItemIcon.Call(uintptr(菜单句柄), uintptr(项ID), 炫彩工具类.StrPtr(文本内容), uintptr(菜单项图标句柄), uintptr(标识), uintptr(insertID))
}
