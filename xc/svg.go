...
//
// pFileName: 文件名.
func XSVG_加载从文件(文件名 string) int {
...
//
// pString: 字符串.
func XSVG_加载从字符串(字符串 string) int {
...
//
// pPassword: zip密码.
func XSVG_加载从ZIP(zip文件名, svg文件名, zip密码 string) int {
...
//
// hModule: 模块句柄, 可填0.
func XSVG_加载从资源ZIP(资源ID int32, svg文件名, zip密码 string, 模块句柄 uintptr) int {
...
//
// hModule: 从指定模块加载.
func XSVG_加载从资源(资源ID int32, 资源类型 string, 从指定模块加载 uintptr) int {
...
//
// nHeight: 高度.
func XSVG_置大小(SVG句柄 int, 宽度, 高度 int32) {
...
//
// pHeight: 接收返回高度.
func XSVG_取大小(SVG句柄 int, 接收返回宽度, 接收返回高度 *int32) {
...
//
// y: y轴偏移.
func XSVG_置偏移(SVG句柄 int, x轴偏移, y轴偏移 int32) {
...
//
// pY: y轴偏移.
func XSVG_取偏移(SVG句柄 int, x轴偏移, y轴偏移 *int32) {
...
//
// y: y轴偏移.
func XSVG_置偏移F(SVG句柄 int, x轴偏移, y轴偏移 float32) {
...
//
// pY: y轴偏移.
func XSVG_取偏移F(SVG句柄 int, x轴偏移, y轴偏移 *float32) {
...
//
// pViewBox: 接收返回视图框.
func XSVG_取视图框(SVG句柄 int, 接收返回视图框 *RECT) {
...
//
// bEnable: 是否自动销毁.
func XSVG_启用自动销毁(SVG句柄 int, 是否自动销毁 bool) {
...
//
// hSvg: SVG句柄.
func XSVG_取引用计数(SVG句柄 int) int32 {
...
//
// alpha: 透明度.
func XSVG_置透明度(SVG句柄 int, 透明度 byte) {
...
//
// bEnable: 是否有效.
func XSVG_置用户填充颜色(SVG句柄 int, AGBR颜色 int, 是否有效 bool) {
...
//
// bEnable: 是否有效.
func XSVG_置用户笔触颜色(SVG句柄 int, AGBR颜色 int, 笔触宽度 float32, 是否有效 bool) {
...
//
// pColor: 返回颜色值, AGBR颜色.
func XSVG_取用户填充颜色(SVG句柄 int, 返回颜色值 *int) bool {
...
//
// pStrokeWidth: .
func XSVG_取用户笔触颜色(SVG句柄 int, 返回颜色值 *int, pStrokeWidth *float32) bool {
...
//
// angle: 转角度.
func XSVG_置旋转角度(SVG句柄 int, 转角度 float32) {
...
//
// hSvg: SVG句柄.
func XSVG_取旋转角度(SVG句柄 int) float32 {
...
func XSVG_置旋转(SVG句柄 int, 角度 float32, 旋转中心点X float32, 旋转中心点Y float32, TRUE bool) {
...
func XSVG_取旋转(SVG句柄 int, 返回角度 *float32, 返回旋转中心点X *float32, 返回旋转中心点Y *float32, 返回TRUE *bool) {
...
//
// bShow: 是否显示.
func XSVG_显示(SVG句柄 int, 是否显示 bool) {
...
//
// pString: 字符串.
func XSVG_加载从字符串W(字符串 string) int {
...
//
// pString: 字符串.
func XSVG_加载从字符串UTF8(字符串 string) int {
...
//
// pPassword: zip密码.
func XSVG_加载从内存ZIP(zip数据 []byte, svg文件名, zip密码 string) int {
