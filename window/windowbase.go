...
// pText:内容文本
// pTitle:标题
func (w *windowBase) X消息框(标题, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, XCStyle 炫彩常量类.Window_Style_) 炫彩常量类.MessageBox_Flag_ {
	return 炫彩基类.X消息框(标题, 内容文本, 标识, w.X取HWND(), XCStyle)
}
...
// pText:内容文本
// pTitle:
func (w *windowBase) X消息框创建(pTitle, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, XCStyle 炫彩常量类.Window_Style_) *ModalWindow {
	p := &ModalWindow{}
	p.X设置句柄(炫彩基类.X消息框_创建(pTitle, 内容文本, 标识, w.X取HWND(), XCStyle))
	return p
}
...
// dwStyle:窗口样式
// dwExStyle:
func (w *windowBase) X消息框创建EX(dwExStyle, 窗口样式 int, 窗口类名, 标题, 内容文本 string, 标识 炫彩常量类.MessageBox_Flag_, XCStyle 炫彩常量类.Window_Style_) *ModalWindow {
	p := &ModalWindow{}
	p.X设置句柄(炫彩基类.X消息框_创建EX(dwExStyle, 窗口样式, 窗口类名, 标题, 内容文本, 标识, w.X取HWND(), XCStyle))
	return p
}
...
// pFun:
// nEvent:事件类型
func (w *windowBase) X注册事件C(事件类型 炫彩常量类.WM_, pFun interface{}) bool {
	return 炫彩基类.X窗口_注册事件C(w.Handle, 事件类型, pFun)
}
...
// pFun:
// nEvent:事件类型
func (w *windowBase) X注册事件C1(事件类型 炫彩常量类.WM_, pFun interface{}) bool {
	return 炫彩基类.X窗口_注册事件C1(w.Handle, 事件类型, pFun)
}
...
// pFun:
// nEvent:事件类型
func (w *windowBase) X移除事件C(事件类型 炫彩常量类.WM_, pFun interface{}) bool {
	return 炫彩基类.X窗口_移除事件C(w.Handle, 事件类型, pFun)
}
...
// pFun:
// nEvent:事件类型
func (w *windowBase) X移除事件CEx(事件类型 炫彩常量类.WM_, pFun uintptr) bool {
	return 炫彩基类.X窗口_移除事件CEx(w.Handle, 事件类型, pFun)
}
...
// bImmediate:
// pRect:需要重绘的区域坐标
func (w *windowBase) X重绘指定区域(需要重绘的区域坐标 *炫彩基类.RECT, bImmediate bool) int {
	return 炫彩基类.X窗口_重绘指定区域(w.Handle, 需要重绘的区域坐标, bImmediate)
}
...
// y:Y坐标
// x:坐标
func (w *windowBase) X移动窗口(坐标, Y坐标 int32) {
	炫彩基类.X窗口_移动窗口(w.Handle, 坐标, Y坐标)
}
...
// color:
// nState:组合状态
func (w *windowBase) X添加背景边框(组合状态 炫彩常量类.Window_State_Flag_, color int, width int) int {
	return 炫彩基类.X窗口_添加背景边框(w.Handle, 组合状态, color, width)
}
...
// color:
// nState:组合状态
func (w *windowBase) X添加背景填充(组合状态 炫彩常量类.Window_State_Flag_, color int) int {
	return 炫彩基类.X窗口_添加背景填充(w.Handle, 组合状态, color)
}
...
// hImage:
// nState:组合状态
func (w *windowBase) X添加背景图片(组合状态 炫彩常量类.Window_State_Flag_, hImage int) int {
	return 炫彩基类.X窗口_添加背景图片(w.Handle, 组合状态, hImage)
}
...
// pTitle:
// position:位置
func (w *windowBase) X通知消息_窗口中弹出(位置 炫彩常量类.Position_Flag_, pTitle, pText string, hIcon int, skin 炫彩常量类.NotifyMsg_Skin_) int {
	return 炫彩基类.X通知消息_窗口中弹出(w.Handle, 位置, pTitle, pText, hIcon, skin)
}
...
// pTitle:
// position:位置
func (w *windowBase) X通知消息_窗口中弹出EX(位置 炫彩常量类.Position_Flag_, pTitle, pText string, hIcon int, skin 炫彩常量类.NotifyMsg_Skin_, bBtnClose, bAutoClose bool, nWidth, nHeight int) int {
	return 炫彩基类.X通知消息_窗口中弹出EX(w.Handle, 位置, pTitle, pText, hIcon, skin, bBtnClose, bAutoClose, nWidth, nHeight)
}
...
func (w *windowBase) X置窗口位置(置顶方式 炫彩常量类.HWND_, x, y, cx, cy int32, uFlags 炫彩常量类.SWP_) int {
	return 炫彩基类.X窗口_置窗口位置(w.Handle, 置顶方式, x, y, cx, cy, uFlags)
}
