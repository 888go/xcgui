...
// 创建列表Ex, 创建列表元素, 使用内置项模板, 自动创建数据适配器.
// x: 元素x坐标.
// y: 元素y坐标.
// cx: 宽度.
// cy: 高度.
...
// 删除全部列, 删除所有的列, 仅删除List的, 数据适配器的列不变.
...
// 取项模板对象, 通过模板项ID, 获取实例化模板项ID对应的对象句柄.
// iItem: 项索引.
// iSubItem: 子项索引.
...
// 取对象所在行, 获取当前对象所在模板实例, 属于列表中哪一个项. 成功返回项索引, 否则返回XC_ID_ERROR.
...
// 取列表头模板对象, 列表头, 通过模板项ID, 获取实例化模板项ID对应的对象句柄.
// iItem: 列表头项ID.
// nTempItemID: 模板项ID.
...
// 取列表头对象所在行, 列表头, 获取当前对象所在模板实例, 属于列表头中哪一个项. 成功返回项索引, 否则返回XC_ID_ERROR.
// hXCGUI: 对象句柄.
