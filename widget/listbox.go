...
// 创建列表框Ex, 创建列表框元素, 使用内置项模板, 自动创建数据适配器.
//
// x: 元素x坐标.
//
// y: 元素y坐标.
...
// 取所在行索引, 获取当前对象所在模板实例, 属于列表中哪一个项(行). 成功返回项索引, 否则返回XC_ID_ERROR.
//
...
// 测试点击项EX, 检测坐标点所在项, 自动添加滚动视图偏移量, 返回项索引.
//
// pPt: 坐标点.
...
// 取模板对象, 通过模板项ID, 获取实例化模板项ID对应的对象句柄, 成功返回对象句柄, 否则返回NULL.
//
// iItem: 项索引.
//
// nTempItemID: 模板项ID.
...
// 创建数据适配器, 创建数据适配器并绑定, 根据绑定的项模板初始化数据适配器的列, 返回适配器句柄.
...
// 添加项文本, XAdTable_AddItemText, 返回项索引.
//
// pText:文本
...
// 添加项文本EX, XAdTable_AddItemTextEx.
//
// pName:名称
//
// pText:文本
...
// 添加项图片EX, XAdTable_AddItemImageEx.
//
// pName:名称
//
// hImage:图片
