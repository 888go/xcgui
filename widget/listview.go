...
// 创建列表视Ex. 创建列表视图元素, 使用内置项模板, 自动创建数据适配器.
// x: 元素x坐标.
// y: 元素y坐标.
// cx: 宽度.
// cy: 高度.
...
// 取模板对象组, 通过模板项ID, 获取实例化模板项ID对应的对象句柄.
// iGroup: 组索引.
// nTempItemID: 模板项ID.
...
// 取对象所在项, 获取当前对象所在模板实例, 属于列表视中哪一个项.
...
// 展开组, 成功返回TRUE否则返回FALSE, 如果状态没有改变返回FALSE.
// iGroup: 组索引.
// bExpand: 是否展开.
...
// 项获取数量, 成功返回项数量, 否则返回 XC_ID_ERROR.
// iGroup: 组索引.
