...
// 启用用户绘制背景, 是否有用户绘制菜单背景, 如果启用XWM_MENU_DRAW_BACKGROUND和XE_MENU_DRAW_BACKGROUND事件有效.
// bEnable: 是否启用.
func (m *Menu) EnableDrawBackground(bEnable bool) {
	xc.XMenu_EnableDrawBackground(m.Handle, bEnable)
}

// 启用用户绘制项, 是否有用户绘制菜单项, 如果启用XWM_MENU_DRAWITEM和XE_MENU_DRAWITEM事件有效.
// bEnable: 是否启用.
