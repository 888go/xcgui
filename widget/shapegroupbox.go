...
// y:Y坐标
// x:坐标
func X创建形状组框(坐标 int, Y坐标 int, 宽度 int, 高度 int, 名称 string, 父对象句柄 int) *ShapeGroupBox {
	p := &ShapeGroupBox{}
	p.X设置句柄(炫彩基类.X形状组框_创建(坐标, Y坐标, 宽度, 高度, 名称, 父对象句柄))
	return p
}
...
func (s *ShapeGroupBox) X取文本偏移(坐标偏移量 *int32, Y坐标偏移量 *int32) int {
	return 炫彩基类.X形状组框_取文本偏移(s.Handle, 坐标偏移量, Y坐标偏移量)
}
