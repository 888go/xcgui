...
func main() {
	...
	// 2.创建窗口
	w = window.I窗口_创建(0, 0, 800, 600, "xc", 0, xcc.I常量_窗口样式_简单|xcc.I常量_窗口样式_按钮_关闭)
	...
	// 注册菜单背景绘制事件
	btn.I事件_绘制菜单背景(onMenuDrawBackground)
	// 注册菜单项绘制事件
	btn.I事件_绘制菜单项(onMenuDrawItem)
	// 注册菜单被选择事件
...
func onBnClick(pbHandled *bool) int {
	// 创建菜单
	menu := widget.I菜单_创建()
	menu.I置项高度(30)       // 设置菜单项高度
	menu.I启用用户绘制背景(true) // 菜单_启用用户绘制背景
	menu.I启用用户绘制项(true)  // 菜单_启用用户绘制项

	// 一级菜单
	menu.I添加项图标(menuid_item1, "item1测试", 0, svgMap[menuid_item1], xcc.I常量_菜单标识_正常)
	menu.I置项宽度(menuid_item1, 100) // 设置菜单宽度
	menu.I添加项图标(menuid_item2, "item2中文", 0, svgMap[menuid_item2], xcc.I常量_菜单标识_正常)
	if item_selected {
		menu.I添加项图标(menuid_item_select, "item_select", 0, svgMap[menuid_item_select], xcc.I常量_菜单标识_勾选)
	} else {
		menu.I添加项(menuid_item_select, "item_select", 0, xcc.I常量_菜单标识_正常)
	}
	menu.I添加项图标(menuid_item3, "item3", 0, svgMap[menuid_item3], xcc.I常量_菜单标识_正常)
	menu.I添加项(-1, "", 0, xcc.I常量_菜单标识_分隔栏) // 分隔栏
	menu.I添加项图标(menuid_item4, "item4", 0, svgMap[menuid_item4], xcc.I常量_菜单标识_正常)
	menu.I添加项图标(menuid_item_disable, "item_disable", 0, svgMap[menuid_item_disable], xcc.I常量_菜单标识_禁用)

	// item1的二级菜单
	menu.I添加项图标(menuid_subitem1, "subitem1", menuid_item1, svgMap[menuid_subitem1], xcc.I常量_菜单标识_正常)
	menu.I添加项图标(menuid_subitem2, "subitem2", menuid_item1, svgMap[menuid_subitem2], xcc.I常量_菜单标识_正常)
...
func onMenuDrawItem(hDraw int, pInfo *xc.Menu_DrawItem_, pbHandled *bool) int {
	*pbHandled = true
	...
	// 绘制分割栏
	if pInfo.NState&xcc.I常量_菜单标识_分隔栏 > 0 {
		xc.XDraw_SetBrushColor(hDraw, xc.ABGR(218, 220, 224, 255))
	...
	if pInfo.NState&xcc.Menu_Item_Flag_Select > 0 {
		...
		/* 		if xc.XMenu_GetNextSiblingItem(pInfo.HMenu, int(pInfo.NID)) != xcc.I常量_错误 {
	...
	// 绘制右三角
	if pInfo.NState&xcc.I常量_菜单标识_弹出 > 0 {
		var pt [3]xc.POINT
	...
	if pInfo.NState&xcc.I常量_菜单标识_禁用 > 0 {
		// 设置被禁用的菜单项文本颜色
	...
	// 绘制菜单项文本
	xc.XDraw_SetTextAlign(hDraw, xcc.I常量_文本对齐_垂直居中|xcc.TextFormatFlag_NoWrap)
